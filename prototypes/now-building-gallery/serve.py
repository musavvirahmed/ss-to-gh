#!/usr/bin/env python3
"""PROTOTYPE server — Building with AI (/ai) card gallery."""
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if raw in ("/", "/ai", "/ai/"):
            return self._send_file(PROTOTYPE / "index.html")
...
def main():
    ...
    try:
        ...
        raise
    print(
        f"Now-building gallery prototype http://{args.bind}:{args.port}/ai "
        f"(?variant=A|B|C|D; site assets at /site/…)",
        flush=True,
    )
    httpd.serve_forever()
